...
The build script we'll be using is contained in the piggie github repository ([contrib/gitian-build.py](https://github.com/derikglobal/piggie/blob/master/contrib/gitian-build.py)). Since this is a completely fresh environment, we haven't yet cloned the piggie repository and will need to fetch this script with the following commands:

```bash
curl -L -O https://raw.githubusercontent.com/piggie/piggie/master/contrib/gitian-build.py
...
- Clone the gitian-builder, gitian.sigs, piggie-detached-sigs, and piggie GitHub repos.
