...
*Setup instructions for a gitian build of piggie Coin using a VM or physical system.*

Gitian is the deterministic build process that is used to build the piggie
...
to the piggie GitHub release page.
...
- [Building piggie Coin](#building-piggie-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/piggie-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/piggie-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/piggie-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/piggie-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/piggie-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/piggie-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/derikglobal/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@mypiggiesbank.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/piggie-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/piggie-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/piggie-osx-*-build.assert
```
