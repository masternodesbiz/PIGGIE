...
CzpiggieStake::CzpiggieStake(const libzerocoin::CoinSpend& spend)
{
...
int CzpiggieStake::GetChecksumHeightFromMint()
{
...
int CzpiggieStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CzpiggieStake::GetChecksum()
{
    return nChecksum;
}

// The zpiggie block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CzpiggieStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
CAmount CzpiggieStake::GetValue()
{
    return denom * COIN;
}
...
bool CzpiggieStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zpiggie staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CzpiggieStake::GetUniqueness()
{
    //The unique identifier for a zpiggie is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CzpiggieStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CzpiggieStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zpiggie that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreatezpiggieOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zpiggie output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zpiggie", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreatezpiggieOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zpiggie output", __func__);
...
bool CzpiggieStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CzpiggieStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzpiggieTracker* zpiggieTracker = pwallet->zpiggieTracker.get();
    CMintMeta meta;
    if (!zpiggieTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zpiggieTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!piggie Stake
bool CpiggieStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CpiggieStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool CpiggieStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CpiggieStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool CpiggieStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
bool CpiggieStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CpiggieStake::GetUniqueness()
{
    //The unique identifier for a piggie stake is the outpoint
...
CBlockIndex* CpiggieStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
