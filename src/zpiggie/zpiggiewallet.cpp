...
CzpiggieWallet::CzpiggieWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpiggie seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzpiggieSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezpiggieSeed_deprecated()) {
                    LogPrintf("%s: Updated zpiggie seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpiggie seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpiggie wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzpiggieWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezpiggieCount(nCountLastUsed);
    else if (!walletdb.ReadzpiggieCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzpiggieWallet::Lock()
{
    seedMaster = 0;
}

void CzpiggieWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzpiggieWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozpiggie(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzpiggieWallet::LoadMintPoolFromDB()
{
...
void CzpiggieWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzpiggieWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzpiggieWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpiggieTracker->HasPubcoinHash(pMint.first)) {
...
bool CzpiggieWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozpiggie(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpiggieTracker which also adds to database
    pwalletMain->zpiggieTracker->Add(dMint, true);
...
void CzpiggieWallet::SeedTozpiggie(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzpiggieWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzpiggieWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WritezpiggieCount(nCountLastUsed);
...
void CzpiggieWallet::GenerateDeterministiczpiggie(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzpiggieWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzpiggieWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzpiggieWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
