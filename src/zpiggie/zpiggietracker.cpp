// Copyright (c) 2018-2019 The piggie developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zpiggie/deterministicmint.h>
#include "zpiggietracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzpiggieTracker::CzpiggieTracker(std::string strWalletFile)
{
...
CzpiggieTracker::~CzpiggieTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzpiggieTracker::Init()
{
...
bool CzpiggieTracker::Archive(CMintMeta& meta)
{
...
bool CzpiggieTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzpiggieTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzpiggieTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzpiggieTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzpiggieTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzpiggieTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzpiggieTracker::GetSerialHashes()
{
...
CAmount CzpiggieTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzpiggieTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzpiggieTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzpiggieTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzpiggieTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzpiggieTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzpiggieTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzpiggieTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzpiggieTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzpiggieTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzpiggieTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzpiggieWallet* zpiggieWallet)
{
    bool iszpiggieWalletInitialized = (NULL != zpiggieWallet);
    CMintMeta meta;
    ...
    if (! iszpiggieWalletInitialized)
        zpiggieWallet = new CzpiggieWallet(strWalletFile);
    meta.isSeedCorrect = zpiggieWallet->CheckSeed(dMint);
    if (! iszpiggieWalletInitialized)
        delete zpiggieWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzpiggieTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzpiggieTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzpiggieTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzpiggieTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzpiggieTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzpiggieTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzpiggieWallet* zpiggieWallet = new CzpiggieWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zpiggieWallet);
        }
        delete zpiggieWallet;
        LogPrint("zero", "%s: added %d dzpiggie from DB\n", __func__, listDeterministicDB.size());
    }
