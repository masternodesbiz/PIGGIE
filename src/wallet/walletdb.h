...
public:
    ...
    bool WritezpiggieSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadzpiggieSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadzpiggieSeed_deprecated(uint256& seed);
    bool ErasezpiggieSeed();
    bool ErasezpiggieSeed_deprecated();

    bool WritezpiggieCount(const uint32_t& nCount);
    bool ReadzpiggieCount(uint32_t& nCount);
