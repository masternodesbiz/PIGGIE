...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    if (nHeight >   8000000) return     0.0 * COIN;
    if (nHeight >   5000000) return     0.5 * COIN;
    if (nHeight >   3000000) return     3 * COIN;
    if (nHeight >   2000000) return     5 * COIN;
    if (nHeight >   1500000) return     10 * COIN;
    if (nHeight >   1200000) return     20 * COIN;
    if (nHeight >    900000) return     15 * COIN;
    if (nHeight >    500000) return     30 * COIN;
    if (nHeight >    300000) return     25  * COIN;
    if (nHeight >    100000) return     20 * COIN;
    if (nHeight >         2) return     25 * COIN;
    if (nHeight >         1) return     18000000 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 PIGGIE tx got MASTERNODE_MIN_CONFIRMATIONS
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        ...
        int nConfHeight = pMNIndex->nHeight + MASTERNODE_MIN_CONFIRMATIONS - 1; // block for 1000 PIGGIE tx -> 1 confirmation
