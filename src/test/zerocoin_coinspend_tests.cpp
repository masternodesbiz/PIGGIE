...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzpiggieWallet *czpiggieWallet = new CzpiggieWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czpiggieWallet->GenerateDeterministiczpiggie(denom, coin, dMint, true);
        czpiggieWallet->UpdateCount();
