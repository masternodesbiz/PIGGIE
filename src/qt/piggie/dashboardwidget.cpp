...
#include "qt/piggie/dashboardwidget.h"
#include "qt/piggie/forms/ui_dashboardwidget.h"
#include "qt/piggie/sendconfirmdialog.h"
#include "qt/piggie/txrow.h"
...
DashboardWidget::DashboardWidget(piggieGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of piggie staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquarepiggie, "square-chart-piggie");
    setCssProperty(ui->labelSquarezpiggie, "square-chart-zpiggie");
    setCssProperty(ui->labelpiggie, "text-chart-piggie");
    setCssProperty(ui->labelzpiggie, "text-chart-zpiggie");
    ui->labelzpiggie->setVisible(false);
    ui->labelSquarezpiggie->setVisible(false);
    ...
    ui->labelAmountzpiggie->setText("0 zpiggie");
    ui->labelAmountzpiggie->setVisible(false);
    ui->labelAmountpiggie->setText("0 piggie");
    setCssProperty(ui->labelAmountpiggie, "text-stake-piggie-disable");
    setCssProperty(ui->labelAmountzpiggie, "text-stake-zpiggie-disable");
    ...
    ui->btnHowTo->setText(tr("How to get piggie"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("piggie")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool ispiggie = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::Stakezpiggie;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair piggie, zpiggie
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            piggie = (pair.first != 0) ? pair.first / 100000000 : 0;
            zpiggie = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalpiggie += pair.first;
            chartData->totalzpiggie += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuespiggie.append(piggie);
        chartData->valueszpiggie.append(zpiggie);

        int max = std::max(piggie, zpiggie);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("piggie");
    set1 = new QBarSet("zpiggie");
    ...
    set0->append(chartData->valuespiggie);
    set1->append(chartData->valueszpiggie);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalpiggie > 0 || chartData->totalzpiggie > 0) {
        setCssProperty(ui->labelAmountpiggie, "text-stake-piggie");
        setCssProperty(ui->labelAmountzpiggie, "text-stake-zpiggie");
    } else {
        setCssProperty(ui->labelAmountpiggie, "text-stake-piggie-disable");
        setCssProperty(ui->labelAmountzpiggie, "text-stake-zpiggie-disable");
    }
    forceUpdateStyle({ui->labelAmountpiggie, ui->labelAmountzpiggie});
    ui->labelAmountpiggie->setText(GUIUtil::formatBalance(chartData->totalpiggie, nDisplayUnit));
    ui->labelAmountzpiggie->setText(GUIUtil::formatBalance(chartData->totalzpiggie, nDisplayUnit, true));
