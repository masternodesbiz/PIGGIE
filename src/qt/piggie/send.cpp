...
#include "qt/piggie/send.h"
#include "qt/piggie/forms/ui_send.h"
#include "qt/piggie/addnewcontactdialog.h"
#include "qt/piggie/qtutils.h"
#include "qt/piggie/sendchangeaddressdialog.h"
#include "qt/piggie/optionbutton.h"
#include "qt/piggie/sendconfirmdialog.h"
#include "qt/piggie/myaddressrow.h"
#include "qt/piggie/guitransactionsutils.h"
...
#include "zpiggiecontroldialog.h"

SendWidget::SendWidget(piggieGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("piggie");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zpiggie");
    setCssProperty(ui->pushRight, "btn-check-right");
    ui->pushRight->setVisible(false);
    ui->pushLeft->setVisible(false);

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (piggie)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter a piggie address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 piggie");
    ...
    setCssProperty(coinIcon, "coin-icon-piggie");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onpiggieSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onpiggieSelected(false);});
...
void SendWidget::refreshView(){
    ...
    if(ui->pushLeft->isChecked()){
        btnText = tr("Send piggie");
        ui->pushButtonAddRecipient->setVisible(true);
    }else{
        btnText = tr("Send zpiggie");
...
void SendWidget::refreshAmounts() {
    ...
    bool iszpiggie = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, iszpiggie));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (iszpiggie ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendpiggie = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendpiggie ? AskPassphraseDialog::Context::Send_piggie : AskPassphraseDialog::Context::Send_zpiggie, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendpiggie) ? send(recipients) : sendzpiggie(recipients)) {
...
bool SendWidget::sendzpiggie(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zpiggie is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zpiggie selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!zpiggieControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = zpiggieControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendzpiggie(
        ...
        inform(tr("zpiggie transaction sent!"));
        zpiggieControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == zpiggie_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zpiggie require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zpiggie transaction
        ...
        emit message("zpiggie transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(ispiggie){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any piggie to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            zpiggieControlDialog *zpiggieControl = new zpiggieControlDialog(this);
            zpiggieControl->setModel(walletModel);
            zpiggieControl->exec();
            ui->btnCoinControl->setActive(!zpiggieControlDialog::setSelectedMints.empty());
            zpiggieControl->deleteLater();
        } else {
            inform(tr("You don't have any zpiggie in your balance to select."));
        }
    }
}
...
void SendWidget::onpiggieSelected(bool _ispiggie){
    ispiggie = _ispiggie;
    setCssProperty(coinIcon, _ispiggie ? "coin-icon-piggie" : "coin-icon-zpiggie");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress piggieAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(piggieAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(piggieAdd.Get(), dialog->getLabel().toStdString(), "send")) {
