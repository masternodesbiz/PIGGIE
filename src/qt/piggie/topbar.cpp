...
#include "qt/piggie/topbar.h"
#include "qt/piggie/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/piggie/lockunlock.h"
#include "qt/piggie/qtutils.h"
#include "qt/piggie/receivedialog.h"
...
TopBar::TopBar(piggieGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountToppiggie, ui->labelAmountTopzpiggie}, "amount-small-topbar");
    setCssProperty({ui->labelAmountpiggie, ui->labelAmountzpiggie}, "amount-topbar");
    setCssProperty({ui->labelPendingpiggie, ui->labelPendingzpiggie, ui->labelImmaturepiggie, ui->labelImmaturezpiggie}, "amount-small-topbar");

    ui->labelTitle2->setVisible(false);
    ui->labelAmountTopzpiggie->setVisible(false);
    ui->labelAmountzpiggie->setVisible(false);
    ui->labelTitle5->setVisible(false);
    ui->labelPendingzpiggie->setVisible(false);
    ui->labelTitle6->setVisible(false);
    ui->labelImmaturezpiggie->setVisible(false);
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    CAmount piggieAvailableBalance = balance + delegatedBalance;

    // zpiggie Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalpiggie = GUIUtil::formatBalance(piggieAvailableBalance, nDisplayUnit);
    QString totalzpiggie = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountToppiggie->setText(totalpiggie);
    ui->labelAmountTopzpiggie->setText(totalzpiggie);

    // Expanded
    ui->labelAmountpiggie->setText(totalpiggie);
    ui->labelAmountzpiggie->setText(totalzpiggie);

    ui->labelPendingpiggie->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzpiggie->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmaturepiggie->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezpiggie->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
