...
#include "qt/piggie/privacywidget.h"
#include "qt/piggie/forms/ui_privacywidget.h"
#include "qt/piggie/qtutils.h"
#include "guiutil.h"
#include "qt/piggie/denomgenerationdialog.h"
#include "qt/piggie/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(piggieGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zpiggie anonymizes your piggie by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zpiggie or convert back to piggie"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 piggie ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zpiggie");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zpiggie");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zpiggie");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zpiggie");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zpiggie");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zpiggie");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zpiggie");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zpiggie");
    ...
    // Only Convert to piggie enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzpiggie->setTitleClassAndText("btn-title-grey", "Total 0 zpiggie");
    ui->btnTotalzpiggie->setSubTitleClassAndText("text-subtitle", "Show denominations of zpiggie owned.");
    ui->btnTotalzpiggie->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select piggie outputs to mint into zpiggie.");
    ...
    connect(ui->btnTotalzpiggie, SIGNAL(clicked()), this, SLOT(onTotalzpiggieClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zpiggie");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of piggie to mint into zpiggie"));
    }else{
        btnText = tr("Convert back to piggie");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zpiggie to convert back into piggie"));
    }
...
void PrivacyWidget::onTotalzpiggieClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzpiggie->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzpiggie->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zpiggie is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zpiggie, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zpiggie").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zpiggie minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackzpiggie(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zpiggie converted back to piggie"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zpiggie </b>";
    ...
    ui->btnTotalzpiggie->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
