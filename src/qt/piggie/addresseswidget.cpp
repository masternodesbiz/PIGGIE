...
#include "qt/piggie/forms/ui_addresseswidget.h"
#include "qt/piggie/addresslabelrow.h"
#include "qt/piggie/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(piggieGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter a piggie address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress piggieAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(piggieAdd.Get(), label.toUtf8().constData(),
                piggieAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
