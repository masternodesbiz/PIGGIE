...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::piggie);
    ...
    if (settings.contains("nAnonymizepiggieAmount") || reset)
        SoftSetArg("-anonymizepiggieamount", settings.value("nAnonymizepiggieAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in piggie.cpp during shotdown to also remove the window positions
