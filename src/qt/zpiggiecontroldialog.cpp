...
#include "ui_zpiggiecontroldialog.h"
...
std::set<std::string> zpiggieControlDialog::setSelectedMints;
std::set<CMintMeta> zpiggieControlDialog::setMints;

bool CzpiggieControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == zpiggieControlDialog::COLUMN_DENOMINATION || column == zpiggieControlDialog::COLUMN_VERSION || column == zpiggieControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


zpiggieControlDialog::zpiggieControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::zpiggieControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zpiggie Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelzpiggie->setProperty("cssClass", "text-main-purple");
    ui->labelzpiggie_int->setProperty("cssClass", "text-main-purple");
...
zpiggieControlDialog::~zpiggieControlDialog()
{
    delete ui;
}

void zpiggieControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zpiggieControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CzpiggieControlWidgetItem* itemDenom(new CzpiggieControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CzpiggieControlWidgetItem *itemMint = new CzpiggieControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zpiggieTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zpiggieTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zpiggie.";
            else if (!mint.isSeedCorrect)
                strReason = "The zpiggie seed used to mint this zpiggie is not the same as currently hold in the wallet";
            else
...
void zpiggieControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zpiggieControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzpiggie_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setzpiggieControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> zpiggieControlDialog::GetSelectedMints()
{
...
void zpiggieControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
