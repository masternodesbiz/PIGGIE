...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? zpiggieModule::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zpiggieTracker->HasMintTx(hash))) {
            //zpiggie stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::Stakezpiggie;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime, wtx.GetTotalSize());
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zpiggie;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
        ...
        isminetype fpiggieoMe = ISMINE_SPENDABLE;
        bool fpiggieoMeDenom = true;
        int nToMe = 0;
        for (const CTxOut& txout : wtx.vout) {
            if (wallet->IsMine(txout)) {
                fpiggieoMeDenom = fpiggieoMeDenom && wallet->IsDenominatedAmount(txout.nValue);
                nToMe++;
            }
            isminetype mine = wallet->IsMine(txout);
            if (mine & ISMINE_WATCH_ONLY) involvesWatchAddress = true;
            if (fpiggieoMe > mine) fpiggieoMe = mine;
        }

        if (fAllFromMeDenom && fpiggieoMeDenom && ((nFromMe * nToMe) != 0)) {
...
bool IszpiggieType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::Stakezpiggie:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zpiggie:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::Stakezpiggie || type == TransactionRecord::MNReward) {
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::Stakezpiggie);
}
