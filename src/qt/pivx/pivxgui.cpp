...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("PIGGIE client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("PIGGIE"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("PIGGIE"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
